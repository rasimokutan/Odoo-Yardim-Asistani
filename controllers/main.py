...
class OdooHelpAssistantController(http.Controller):
    ...
    @http.route("/odoo_help_assistant/chat/bootstrap", type="json", auth="user")
    def chatbot_bootstrap(self):
        service = self._service()
        can_use_chatbot = request.env.user.has_group("base.group_user")
        return {
    ...
    @http.route("/odoo_help_assistant/chat/session/create", type="json", auth="user")
    def create_session(self):
        session = request.env["odoo.chatbot.session"].create_user_session()
        return session.get_payload(include_messages=True)

    @http.route("/odoo_help_assistant/chat/session/load", type="json", auth="user")
    ...
    @http.route("/odoo_help_assistant/chat/send", type="json", auth="user")
